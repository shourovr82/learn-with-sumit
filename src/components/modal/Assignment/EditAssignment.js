...
import { useEditAssignmentMutation } from '../../../redux/features/assignment/assignmentApi';
// import { Listbox, Transition } from '@headlessui/react'


const EditAssignment = ({ isEditOpen, setEditOpen, assignment }) => {
  ...
  const handleUpdateAssignment = (data) => {
    const submitAssignment = {
      ...
      video_id: selected?.id ? selected?.id : assignment?.id,
      video_title: selected?.title ? selected?.title : assignment?.title,
      totalMark: Number(data?.assignmentMark)
    }
    editAssignment({ id: assignment?.id, submitAssignment })
    toast.success('এসাইনমেন্ট আপডেট সম্পন্ন হয়েছে ')
    setEditOpen(false)
                    ...
                    <div className="mt-5">
                      <form onSubmit={handleSubmit(handleUpdateAssignment)}>
                        <div className='mt-3'>
                          ...
                          <div className="">
                            <Listbox value={selected ? selected : assignment} onChange={handleChangeVideoTitle}>
                              <div className="relative mt-1">
                                ...
                                <Listbox.Button className="relative  w-full  rounded-lg !bg-[#1e293b] py-2 pl-3 pr-10 text-left shadow-md focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm">
                                  <span className="block truncate">{selected?.title ? selected?.title : assignment?.title}</span>
                        ...
                        <div className='mt-3'>
                          ...
                          <input defaultValue={assignment?.title} {...register("assignmentTitle", { required: 'এসাইনমেন্ট এর টাইটেল লেখা  বাধ্যতামূলক ' })} type="text" id='title' className='rounded-md   mt-2 placeholder:text-xs placeholder:text-slate-400 focus:ring-2 focus:ring-[#114a72] ring-inset focus:border-0 focus:outline-none p-2 w-full bg-[#1e293b]' placeholder='Assignment Title...' />
                          {
                        ...
                        <div className='mt-3'>
                          ...
                          <input defaultValue={assignment?.totalMark} {...register("assignmentMark", {
                        ...
                        <div className='mt-5 '>
                          ...
                          <button className='!bg-[#0284c7] font-bold font-serif hover:!bg-[#32aeec] px-5 py-2 rounded-full' type='submit'>এসাইনমেন্ট আপডেট   করুন  </button>
                        </div>
