...
import { useAddQuizMutation, useEditQuizMutation } from '../../../redux/features/quiz/quizApi';

const EditQuizModal = ({ editData, setEditQuizOpen, editQuizOpen }) => {
  const [selected, setSelected] = useState(editData || {})
  ...
  const handleAddNewQuiz = (data) => {
    if (!selected?.id) {
      ...
      const newQuizData = {
        ...
        video_title: selected?.title ? selected?.title : selected?.video_title,
        options
      }
      const id = editData?.id;
      editQuiz({ id, newQuizData })
      setEditQuizOpen(false)
  ...
  return (
    <>
      ...
      <Transition appear show={editQuizOpen} as={Fragment}>
                  ...
                  <div className="mt-5">
                    <form onSubmit={handleSubmit(handleAddNewQuiz)}>
                      <div className='mt-3'>
                        ...
                        <div className="">
                          <Listbox value={selected ? selected : editData} onChange={handleChangeQuizTitle}>
                            <div className="relative mt-1">
                              ...
                              <Listbox.Button className="relative  w-full  rounded-lg !bg-[#1e293b] py-2 pl-3 pr-10 text-left shadow-md focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm">
                                <span className="block truncate">{selected?.title ? selected?.title : editData?.video_title}</span>
                      ...
                      <div className='mt-3'>
                        ...
                        <input defaultValue={editData?.question} {...register("question", { required: 'কুইজ এর টাইটেল দিতে হবে   ' })} type="text" id='question' className='rounded-md   mt-2 placeholder:text-xs placeholder:text-slate-400 focus:ring-2 focus:ring-[#114a72] ring-inset focus:border-0 focus:outline-none p-2 w-full bg-[#1e293b]' placeholder='Quiz  Title...' />
                        {
                      ...
                      <div className='mt-3'>
                        ...
                        <div className=' flex items-center justify-between'>
                          <input defaultValue={editData?.options[0]?.option} {...register("question1", { required: 'কুইজ এর প্রশ্ন দিতে হবে ' })} type="text" id='question1' className='rounded-md   mt-2 placeholder:text-xs placeholder:text-slate-400 focus:ring-2 focus:ring-[#114a72] ring-inset focus:border-0 focus:outline-none p-2 w-[75%] bg-[#1e293b]' placeholder='Quiz Question...' />

                          <div className="flex  items-center gap-2">
                            <label htmlFor="isCorrect1" className='text-sm mt-1.5  text-slate-400'>Correct?</label>
                            <select defaultValue={true} {...register('isCorrect1')} className='px-3 hover:ring-2 rounded-md  duration-300 ease-in outline-none cursor-pointer bg-[#1e293b] py-2 mt-2 text-[#818fa5]' name="isCorrect1" id="isCorrect1">
                      ...
                      <div className='mt-3'>
                        ...
                        <div className=' flex items-center justify-between'>
                          <input defaultValue={editData?.options[1]?.option} {...register("question2", { required: 'কুইজ এর প্রশ্ন দিতে হবে  ' })} type="text" id='question2' className='rounded-md   mt-2 placeholder:text-xs placeholder:text-slate-400 focus:ring-2 focus:ring-[#114a72] ring-inset focus:border-0 focus:outline-none p-2 w-[75%] bg-[#1e293b]' placeholder='Quiz Question...' />
                      ...
                      <div className='mt-3'>
                        ...
                        <div className=' flex items-center justify-between'>
                          <input defaultValue={editData?.options[2]?.option} {...register("question3", { required: 'কুইজ এর প্রশ্ন দিতে হবে  ' })} type="text" id='question3' className='rounded-md   mt-2 placeholder:text-xs placeholder:text-slate-400 focus:ring-2 focus:ring-[#114a72] ring-inset focus:border-0 focus:outline-none p-2 w-[75%] bg-[#1e293b]' placeholder='Quiz Question...' />
                      ...
                      <div className='mt-3'>
                        ...
                        <div className=' flex items-center justify-between'>
                          <input defaultValue={editData?.options[3]?.option} {...register("question4", { required: 'কুইজ এর প্রশ্ন দিতে হবে ' })} type="text" id='question4' className='rounded-md   mt-2 placeholder:text-xs placeholder:text-slate-400 focus:ring-2 focus:ring-[#114a72] ring-inset focus:border-0 focus:outline-none p-2 w-[75%] bg-[#1e293b]' placeholder='Quiz Question...' />

                          <div className="flex  items-center gap-2">
                            <label htmlFor="isCorrect4" className='text-sm mt-1.5  text-slate-400'>Correct?</label>
                            <select defaultValue={editData?.options[3]?.isCorrect ? editData?.options[3]?.isCorrect : true} {...register('isCorrect4')} className='px-3 hover:ring-2 rounded-md  duration-300 ease-in outline-none cursor-pointer bg-[#1e293b] py-2 mt-2 text-[#818fa5]' name="isCorrect4" id="isCorrect4">
                              <option value={true}>{`${editData?.options[3]?.isCorrect === 'true' ? 'true' : 'true'}`}</option>
                              <option value={false}>false</option>
                            </select>
                          </div>
                      ...
                      <div className='mt-5 '>
                        <button className='!bg-[#0284c7] font-semibold text-sm font-serif hover:!bg-[#32aeec] px-5 py-2 rounded-full' type='submit'>সাবমিট করুন  </button>
                      </div>
